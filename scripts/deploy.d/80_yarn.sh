...
init_d10_theme_install() {
	...
	chmod -R u+w "${DIR_THEME}" 2>/dev/null || true  ## droits en écriture pour installations & mises à jour
...
init_d10_theme_build() {
	...
	chmod -R u+w "${DIR_THEME}" 2>/dev/null || true  ## droits en écriture pour compilations des assets
